 ...
 * state transition rules for {@link Lifecycle#start()} and {@link Lifecycle#stop()}
 */
public abstract class LifecycleBase implements Lifecycle {
     ...
     * {@link #stopInternal()} or {@link #destroyInternal()} be re-thrown for the caller to handle or will it be logged instead?
     *
    ...
    public final synchronized void init() throws LifecycleException {
        // 检测当前组件的状态是不是 NEW (新建)，如果不是就调用 invalidTransition 方法来将当前的状态转换过程终止，而 invalidTransition 的实现是抛出了 org.apache.catalina.LifecycleException 异常。
        // 接着调用了 setStateInternal 方法将状态设置为 INITIALIZING（正在初始化）
        if (!state.equals(LifecycleState.NEW)) {
            invalidTransition(Lifecycle.BEFORE_INIT_EVENT);
        }

        try {
            // 将组件的状态改为 INITIALIZING
            setStateInternal(LifecycleState.INITIALIZING, null, false);
            // init 模板方法的钩子，子类可以通过实现 initInternal 方法来纳入初始化的流程。
            initInternal();
            // 将组件的状态改为 INITIALIZED (已初始化)
    ...
    public final synchronized void start() throws LifecycleException {
        // 检测当前组件的状态是不是 STARTING_PREP(准备启动),STARTING（正在启动）,STARTED（已启动）.如果是这三个状态中的任何一个，则抛出 LifecycleException。
        ...
        // 检查其实主要是为了保证组件状态的完整性，在正常启动的流程中，应该是不会出现没有初始化就启动，或者还没启动就已经失败的情况。
        if (state.equals(LifecycleState.NEW)) {
            init();
        ...
        try {
            // 设置组件的状态为 STARTING_PREP（准备启动状态）
            setStateInternal(LifecycleState.STARTING_PREP, null, false);
            // start 模板方法的钩子方法，子类通过实现 startInternal 这个方法来纳入到组件启动的流程中来。
            startInternal();

            // 一些状态检查，然后最终将组件的状态设置为 STARTED(已启动)
     ...
     * will be called on the failed component but the parent component will continue to start normally.
     *
    ...
    public final synchronized void stop() throws LifecycleException {
        ...
        try {
            if (state.equals(LifecycleState.FAILED)) {
                ...
                // component as available but do ensure the BEFORE_STOP_EVENT is fired
