...
public final class Bootstrap {
    ...
    // https://blog.csdn.net/qq_30835699/article/details/82429580
    // catalina.base(工作目录)：指向每个Tomcat目录私有信息的位置，就是 conf、logs、temp、webapps 和 work 的父目录。
    ...
    public static void main(String args[]) {
        synchronized (daemonLock) {
            if (daemon == null) {
                // Don't set daemon until init() has completed. 主要执行static中的设置Catalina.home和Catalina.base的值
                Bootstrap bootstrap = new Bootstrap();
                try {
                    // 主要完成类加载器的初始化，包括common，catalina，share三个类加载器，并设置catalina的父类加载器。
                    bootstrap.init();
                } catch (Throwable t) {
                    handleThrowable(t);
                    t.printStackTrace();
    ...
    static {
        ...
        String userDir = System.getProperty("user.dir") + File.separator + "tomcat" + File.separator + "home";
        ...
        base = System.getProperty("user.dir") + File.separator + "tomcat" + File.separator + "base";
        if (base == null) {
        ...
        System.setProperty("java.io.tmpdir",System.getProperty("user.dir") + File.separator + "tomcat" + File.separator + "base"+ File.separator + "temp");
    }
    ...
    public void init() throws Exception {
        /**
         * commonLoader：Tomcat最基本的类加载器，直接继承systemLoader，加载路径中的class可以被tomcat容器本身以及各个webapp访问。
         *              加载路径为：common.loader，默认执行${catalina.home}/lib下的包。
        ...
        // 2.设置当前线程的类加载器为 catalinaLoader，即当前线程只会加载 tomcat 私有的 jar 包
        Thread.currentThread().setContextClassLoader(catalinaLoader);

        // 3.设置 Java SecutityManager
