...
/**
 * Standard implementation of the <b>Server</b> interface, available for use (but not required) when deploying and starting Catalina.
 *
...
public final class StandardServer extends LifecycleMBeanBase implements Server {
    ...
    /**
     * Global naming resources context. JNDI 的 context
     */
    private javax.naming.Context globalNamingContext = null;

    /**
     * Global naming resources. JNDI 的 resource
     */
    private NamingResourcesImpl globalNamingResources = null;

    /**
     * The naming context listener for this web application. 作用于 web application 的 JDNI 监听器
     */
    private final NamingContextListener namingContextListener;


    /**
     * The port number on which we wait for shutdown commands. 用于等待 shutdown 命令的端口号
     */
    ...
    /**
     * The address on which we wait for shutdown commands. 用于等待 shutdown 命令的地址
     */
    private String address = "localhost";

    /**
     * The shutdown command string we are looking for. shutdown 服务在等待的命令
     */
    ...
    /**
     * The set of Services associated with this Server. Server 下的 service 集合
     */
    ...
    /**
     * Thread that currently is inside our await() method. 执行 await() 的线程
     */
    private volatile Thread awaitThread = null;

    /**
     * Server socket that is used to wait for the shutdown command. 用于等待 shutdown 的 socket 对象
     */
    ...
    protected void startInternal() throws LifecycleException {
        ...
        // Start our defined Services. 调用 Service 的 start() 方法，启动 Service
        synchronized (servicesLock) {
            for (int i = 0; i < services.length; i++) {
                services[i].start();
            }
    ...
    protected void initInternal() throws LifecycleException {
        ...
        // Register the MBeanFactory. 注册 MBean，使通过 JMX 能够监控
        MBeanFactory factory = new MBeanFactory();
        factory.setContainer(this);
        onameMBeanFactory = register(factory, "type=MBeanFactory");

        // Register the naming resources. 调用 JNDI 的 init 方法
        globalNamingResources.init();

        // Populate the extension validator with JARs from common and shared class loaders
        // 对 common 和 shared 目录下的 jar 包进行校验，如果给出的 jar 文件包含 MANIFEST，将被添加到 container 的 manifest 资源中。
        if (getCatalina() != null) {
        ...
        // Initialize our defined Services. 执行 Service 的 init 方法
        for (int i = 0; i < services.length; i++) {
            services[i].init();
        }
    }
