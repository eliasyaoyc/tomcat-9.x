...
public abstract class AbstractProtocol<S> implements ProtocolHandler, MBeanRegistration {
     ...
     * to use a random port as the port will change if stop(), start() is called.
     */
     ...
     * ProtocolHandler implementation (ProtocolHandler using NIO, requires NIO Endpoint etc.).
     */
    ...
    protected static class ConnectionHandler<S> implements AbstractEndpoint.Handler<S> {
        ...
        public SocketState process(SocketWrapperBase<S> wrapper, SocketEvent status) {
            ...
            // Timeouts are calculated on a dedicated thread and then dispatched. Because of delays in the dispatch process, the
            // timeout may no longer be required. Check here and avoid unnecessary processing.
            ...
            try {
                if (processor == null) {
                    ...
                    if (negotiatedProtocol != null && negotiatedProtocol.length() > 0) {
                        ...
                        if (upgradeProtocol != null) {
                            ...
                            // TODO:
                            // OpenSSL 1.0.2's ALPN callback doesn't support failing the handshake with an error if no
                            // protocol can be negotiated. Therefore, we need to fail the connection here. Once this is fixed,
                            // replace the code below with the commented out block.
                            if (getLog().isDebugEnabled()) {
                ...
                do {
                    ...
                    if (state == SocketState.UPGRADING) {
                        ...
                        if (upgradeToken == null) {
                            ...
                            // Associate with the processor with the connection
                            wrapper.setCurrentProcessor(processor);
                            // Initialise the upgrade handler (which may trigger some IO using the new protocol which is why the lines above are necessary)
                            // This cast should be safe. If it fails the error handling for the surrounding try/catch will deal with it.
                ...
                if (state == SocketState.LONG) {
                    ...
                    // socket associated with the processor. Exact requirements depend on type of long poll
                    ...
                    // Sendfile in progress. If it fails, the socket will be closed. If it works, the socket either be added to the
                    // poller (or equivalent) to await more data or processed if there are any pipe-lined requests remaining.
                } else if (state == SocketState.UPGRADED) {
                    // Don't add sockets back to the poller if this was a non-blocking write otherwise the poller may trigger
                    // multiple read events which may lead to thread starvation in the connector. The write() method will add this socket
            ...
            // Future developers: if you discover any other rare-but-nonfatal exceptions, catch them here, and log as above.
            catch (OutOfMemoryError oome) {
                // Try and handle this here to give Tomcat a chance to close the connection and prevent clients waiting until they time out.
                // Worst case, it isn't recoverable and the attempt at logging will trigger another OOME.
